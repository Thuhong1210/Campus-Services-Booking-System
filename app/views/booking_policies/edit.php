<div class="container-fluid px-4 py-4">
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h1 class="h3 fw-bold mb-0 text-gradient"><i class="bi bi-pencil-square me-2 text-primary"></i><?= e(__('Edit Booking Policy')) ?></h1>
      <p class="text-muted mb-0"><?= e(__('Modify and tune active booking policies for resource categories')) ?></p>
    </div>
  ...
  <div class="row g-4">
    <!-- Form Card -->
    <div class="col-lg-8">
      <form id="policyForm" method="POST" action="<?= url('index.php?page=booking-policies&action=update&id=' . $policy['id']) ?>" class="card shadow-premium border-0 rounded-4 p-4">
        <?= csrf_field() ?>
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <input name="policy_name" id="policyName" class="form-control rounded-3 shadow-sm" value="<?= e($policy['policy_name']) ?>" required>
          </div>
          ...
          <div class="col-md-6">
            ...
            <select name="category_id" id="policyCategory" class="form-select rounded-3 shadow-sm" required>
              <?php foreach ($categories as $c): ?>
                <option value="<?= $c['id'] ?>" data-name="<?= e($c['category_name']) ?>" <?= $policy['category_id'] == $c['id'] ? 'selected' : '' ?>>
                  <?= e($c['category_name']) ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="col-md-6 my-3">
            <label class="form-label fw-medium d-flex justify-content-between">
              ...
              <span class="text-primary fw-bold" id="maxDurationVal"><?= e($policy['max_duration_hours']) ?> hrs</span>
            </label>
            <input type="range" class="form-range" min="0.5" max="12" step="0.5" name="max_duration_hours" id="maxDuration" value="<?= e($policy['max_duration_hours']) ?>">
          </div>
          ...
          <div class="col-md-6 my-3">
            <label class="form-label fw-medium d-flex justify-content-between">
              ...
              <span class="text-primary fw-bold" id="weeklyQuotaVal"><?= e($policy['weekly_quota'] ?? 5) ?> bookings</span>
            </label>
            <input type="range" class="form-range" min="1" max="20" step="1" name="weekly_quota" id="weeklyQuota" value="<?= e($policy['weekly_quota'] ?? 5) ?>">
          </div>
          ...
          <div class="col-md-6 my-3">
            <label class="form-label fw-medium d-flex justify-content-between">
              ...
              <span class="text-primary fw-bold" id="peakLimitVal"><?= e($policy['max_peak_slots_per_week'] ?? 2) ?> slots</span>
            </label>
            <input type="range" class="form-range" min="0" max="10" step="1" name="max_peak_slots_per_week" id="peakLimit" value="<?= e($policy['max_peak_slots_per_week'] ?? 2) ?>">
          </div>
          ...
          <div class="col-md-6 my-3">
            <label class="form-label fw-medium d-flex justify-content-between">
              ...
              <span class="text-primary fw-bold" id="cancelDeadlineVal"><?= e($policy['cancellation_deadline_hours'] ?? 24) ?> hrs</span>
            </label>
            <input type="range" class="form-range" min="1" max="72" step="1" name="cancellation_deadline_hours" id="cancelDeadline" value="<?= e($policy['cancellation_deadline_hours'] ?? 24) ?>">
          </div>
          ...
          <div class="col-md-6">
            ...
            <select name="requires_approval" id="requiresApproval" class="form-select rounded-3 shadow-sm">
              <option value="0" <?= $policy['requires_approval'] == 0 ? 'selected' : '' ?>>No (Auto-approves if criteria met)</option>
              <option value="1" <?= $policy['requires_approval'] == 1 ? 'selected' : '' ?>>Yes (Always requires manual signoff)</option>
            </select>
          </div>
          ...
          <div class="col-md-6" id="autoApprovalWrapper">
            ...
            <select name="auto_approval_enabled" id="autoApproval" class="form-select rounded-3 shadow-sm">
              <option value="1" <?= ($policy['auto_approval_enabled'] ?? 0) == 1 ? 'selected' : '' ?>>Yes</option>
              <option value="0" <?= ($policy['auto_approval_enabled'] ?? 0) == 0 ? 'selected' : '' ?>>No</option>
            </select>
          </div>
        </div>
        ...
        <div class="mt-4 pt-3 border-top d-flex gap-2">
          <button type="submit" class="btn btn-primary rounded-pill px-4 py-2 fw-semibold shadow-premium">
            <i class="bi bi-save me-1"></i><?= e(__('Update Policy')) ?>
          </button>
